...
{
    public partial class GRNPM : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                DataSet daygrn = objBs.daygrnValmp();
                ddldaygrn.DataSource = daygrn.Tables[0];
                ddldaygrn.DataTextField = "DayGRNmp";
                ddldaygrn.DataValueField = "id";
                ddldaygrn.DataBind();
                ddldaygrn.Items.Insert(0, "Select GRN");

                FirstEntry = "yes";
                DataSet dNo = objBs.GRNNOpm(Convert.ToInt32(lblUserID.Text));
                {
                ...
                if (iStock != "" && iStock != null)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        txtQty.Text = Convert.ToDouble(ds.Tables[0].Rows[0]["Available_QTY"]).ToString(""+qtysetting+"");
                        ...
                        txtCustUnitPrice.Text = Decimal.Round(CusUnit, 2).ToString(""+ratesetting+"");
        ...
        protected void ddldaygrn_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataSet ds = objBs.daygrnValid1(Convert.ToInt32(ddldaygrn.SelectedValue));
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            ...
            DataSet ds = objBs.getgrndatetime1(txttodate.Text);
            string caption = " <h4><b>" + " Store :  " + BranchNAme + " " + StoreName + " Generate On " + DateTime.Now.ToString("MM/dd/yyyy hh:mm tt") + " </b></h4> ";
        ...
        protected void ddlSubCategory_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (sTableName != "admin")
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    lblavastock.Text = Convert.ToDouble(ds.Tables[0].Rows[0]["Available_Qty"]).ToString(""+qtysetting+"");
                ...
                {
                    lblavastock.Text = Convert.ToDouble(0).ToString("" + qtysetting + "");
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('Branch Done Denomination.Not Allow To Make Any GRN (+)(-).Please Contact Administrator.Thank You!!!');", true);
                return;
            }
            else
            {
            ...
            if (txtname.Text.Trim() != "")
            {
                ...
                Decimal ichecked = 0;
                if (radbtnlist.SelectedValue == "-")
                {
                    if (Convert.ToDecimal(txtQty.Text) > Convert.ToDecimal(lblavastock.Text))
                    {
                        lblerror.Text = "Avaliable Stock is Greater Than Entered Qty";
                        return;
                    }
                }
                else
                        ...
                        if (btnAdd.Text == "Save")
                        {
                            ...
                            //else
                            {
                                ...
                                if (dsStock.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    iSuccess = objBs.updatePrintstockdetpm(Convert.ToInt32(ddlcategory.SelectedValue), Convert.ToInt32(ddlSubCategory.SelectedValue), Convert.ToDouble(dAvbQty), sTableName, Convert.ToDecimal(txtQty.Text), DateTime.Now.ToString("MM/dd/yyyy"), Convert.ToInt32(lblUserID.Text), stockid, Convert.ToInt32(txtgrnno.Text), txtname.Text, radbtnlist.SelectedValue);
                                    //  Response.Redirect("../Accountsbootstrap/GRNPM.aspx");
                                }
                                else
                                {

                                    iSuccess = objBs.StockOnlypm(sTableName, Convert.ToInt32(lblUserID.Text), Convert.ToInt32(ddlcategory.SelectedValue), Convert.ToInt32(ddlSubCategory.SelectedValue), Convert.ToDouble(txtQty.Text), Convert.ToDouble(txtQty.Text), Convert.ToDouble(0), Convert.ToDouble(0), Convert.ToDouble(0), Convert.ToDouble(0), Convert.ToString(DateTime.Now.ToString("MM/dd/yyyy")), 1, Convert.ToInt32(txtgrnno.Text), txtname.Text, radbtnlist.SelectedValue);
                                }
                            }
                            txtQty.Text = "";
                            lblerror.Text = "saved";
